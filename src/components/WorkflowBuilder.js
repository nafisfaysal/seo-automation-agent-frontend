...
import { Box, Typography, Button, Grid, Paper, IconButton, Tooltip, Divider, Input } from '@mui/material';
...
const nodeOptions = [
  ...
  { id: 'extractSEMRush', label: 'SEMRush Keyword Research', icon: '🔍', description: 'Extract keywords using SEMRush API or upload your own data.', canUploadFile: true },
  ...
  { 
    id: 'customContentOutline', 
    label: 'Custom Content Outline', 
    icon: '📝', 
    description: 'Upload a custom content outline in Excel or CSV format.',
    canUploadFile: true
  },
];
...
function WorkflowBuilderContent() {
  ...
  const handleFileUpload = useCallback((event, nodeId) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        setUploadedFiles(prev => ({
          ...prev,
          [nodeId]: { fileName: file.name, data: jsonData }
        }));

        // Update the node to show the uploaded file name
        setNodes(nds => nds.map(node => 
          node.id === nodeId 
            ? { ...node, data: { ...node.data, uploadedFile: file.name } }
            : node
        ));
      };
  ...
  const handleRunWorkflow = useCallback(() => {
    ...
    setWorkflowData(/* Your final workflow data */);
    
    navigate('/analysis');
  }, [navigate]);

  useEffect(() => {
    if (workflowCompleted && workflowData) {
      // Generate and download Excel file
      const wb = XLSX.utils.book_new();
      
      // Add sheets for each section of the analysis
      ...
      link.download = 'seo_analysis_results.xlsx';
      link.click();
  ...
  }, [workflowCompleted, workflowData]);
